...
function DeploymentModeStep({
  ...
  // The mode cards + copy live in the shared DeploymentModePicker (also used by
  // the post-setup Settings surface). In the wizard, picking a mode applies it
  // immediately — no confirm, since setup isn't live yet.
  return (
    <DeploymentModePicker
      mode={state.mode}
      firewallCapable={state.firewallCapable}
      busy={busy === 'deployment_mode'}
      onSelect={onSetMode}
    />
  );
}
