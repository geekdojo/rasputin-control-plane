...
type DockerComposeSupervisorConfig struct {
	...
	// starter-rules YAML against VM and writes each alert's state back
	// to VM as ALERTS series, which the api reads (rule_alerts.go). It
...
type DockerComposeSupervisor struct {
	...
	// grafanaProvisioning is what writeGrafanaConfig lays down besides
	// grafana.ini. Always defaultGrafanaProvisioning in production; a
	// functional test swaps in the pre-fix files to prove an existing
	// cluster converges.
	grafanaProvisioning []grafanaFile
}
...
// grafanaFile is one rendered Grafana config file: its path under StateDir
// and its contents.
...
var (
	grafanaIniPath              = filepath.Join(grafanaConfigSubdir, grafanaIniFile)
	grafanaDatasourcesPath      = filepath.Join(grafanaConfigSubdir, "provisioning", "datasources", "all.yaml")
	grafanaDashboardsPath       = filepath.Join(grafanaConfigSubdir, "provisioning", "dashboards", "all.yaml")
	grafanaStarterDashboardPath = filepath.Join(grafanaConfigSubdir, "dashboards", "cluster-overview.json")
)

// defaultGrafanaProvisioning is every provisioned datasource and dashboard
// file. A new dashboard goes here, and TestProvisionedDashboards_* then
// checks each of its datasource references against the datasources file.
var defaultGrafanaProvisioning = []grafanaFile{
	{grafanaDatasourcesPath, grafanaDatasourcesYAML},
	{grafanaDashboardsPath, grafanaDashboardsYAML},
	{grafanaStarterDashboardPath, starterDashboardJSON},
}

func (s *DockerComposeSupervisor) grafanaProvisioningFiles() []grafanaFile {
	if s.grafanaProvisioning != nil {
		return s.grafanaProvisioning
	}
	return defaultGrafanaProvisioning
}

// grafanaConfigPaths is every file Grafana reads from the api, absolute —
// the input to Grafana's config digest.
func (s *DockerComposeSupervisor) grafanaConfigPaths() []string {
	out := []string{filepath.Join(s.cfg.StateDir, grafanaIniPath)}
	for _, f := range s.grafanaProvisioningFiles() {
		out = append(out, filepath.Join(s.cfg.StateDir, f.path))
	}
	return out
}
...
// Fixed datasource uids. Every provisioned dashboard references its
// datasource by one of these.
//
// Before 2026-09-18 the datasources had no uid, so Grafana derived one from
// the name (VictoriaMetrics -> P4169E866C3094E38 on every cluster), while the
// starter dashboard hard-coded PBFA97CFB590B2093 — the uid Grafana derives
// for a datasource named "Prometheus" (measured on 11.5.1), which Rasputin
// never provisioned.
// Every panel failed with "Datasource PBFA97CFB590B2093 was not found" from
// 2026-06-02 on. Pinning the uid here makes the reference a contract the
// unit tests can check, instead of a value copied out of some other Grafana.
//
// On an existing cluster Grafana matches the provisioned datasource to its
// stored row by NAME and rewrites the uid in place (same row id; verified on
// grafana/grafana:11.5.1 — see TestGrafanaDatasourceUID_TransitionFromAutoUID),
// so nothing needs deleting first.
const (
	vmDatasourceUID   = "rasputin-victoriametrics"
	lokiDatasourceUID = "rasputin-loki"
)

// vmNodeLabel is the label VMSink (encodePromText) puts on every rasputin_*
// series: the node id bound to the bus subject. dashboardNodeVar is the
// starter dashboard's node variable — and so the var-<name> URL parameter
// the UI's "in Grafana" links send. Both are "nodeId"; the tests hold them
// to the sink's real output and to the UI's links.
const (
	vmNodeLabel      = "nodeId"
	dashboardNodeVar = "nodeId"
)
...
//
// The nodeId variable filters every panel to the chosen nodes. Its name is
// the URL parameter the UI's "in Grafana" links send (var-nodeId=<node id>),
// and it is also the label VMSink puts on every series (encodePromText), so
...
// range. It reads 1 for one reporting node, and noValue shows 0 (not
// "No data") when the selection has none. Not "count(...) or vector(0)"
// over a range: the newest step of a range query can have no sample yet,
// and vector(0) turns that gap into a last value of 0 while every node is
// reporting (measured on VictoriaMetrics v1.103.0 through Grafana 11.5.1).
const starterDashboardJSON = `{
  ...
  "templating": {
    "list": [
      {
        "name": "` + dashboardNodeVar + `",
        "label": "Node",
        "type": "query",
        "datasource": {"type": "prometheus", "uid": "` + vmDatasourceUID + `"},
        "definition": "label_values(rasputin_cpu_percent, ` + vmNodeLabel + `)",
        "query": {"query": "label_values(rasputin_cpu_percent, ` + vmNodeLabel + `)", "refId": "PrometheusVariableQueryEditor-VariableQuery"},
        "refresh": 2,
        "sort": 1,
        "multi": true,
        "includeAll": true,
...
//
// The rules carry no annotations: vmalert has no notifier, so
// annotations would never leave it. The operator-facing text for each
// alert is RuleAlertSummary, rendered by the api.
var vmalertRulesYAML = `# Generated by rasputin-api obs.DockerComposeSupervisor — do not hand-edit.
groups:
  - name: rasputin-default
    interval: ` + VMAlertEvaluationInterval.String() + `
    rules:
      - alert: NodeDown
...
func (s *DockerComposeSupervisor) renderCompose() ([]byte, error) {
	...
	data := composeData{
		...
		// Every Grafana file, not just grafana.ini: Grafana reads datasource
		// provisioning only at startup, so a changed datasources file that
		// does not recreate the container never applies.
		GrafanaConfigHash: configHash(s.grafanaConfigPaths()...),
		...
		VMAlertInterval:      VMAlertEvaluationInterval.String(),
		VMAlertEvalDelay:     VMAlertEvalDelay.String(),
	}
	var buf bytes.Buffer
...
type composeData struct {
	...
	// VMAlertInterval and VMAlertEvalDelay render VMAlertEvaluationInterval
	// and VMAlertEvalDelay: the api's staleness bound on the ALERTS it reads
	// back (RuleAlertFreshness) is derived from them.
	VMAlertInterval  string
	VMAlertEvalDelay string
}
...
{{- if .EnableVMAlert }}
  ...
  vmalert:
    ...
    command:
      ...
      # No notifier: vmalert only evaluates and writes ALERTS series to VM,
      # which the api reads (rule_alerts.go). vmalert refuses to start with
      # alerting rules and no notifier flag, so blackhole is set explicitly.
      - "-notifier.blackhole"
      - "-evaluationInterval={{.VMAlertInterval}}"
      - "-rule.evalDelay={{.VMAlertEvalDelay}}"
    volumes:
