...
//
// CodeQL flags this as go/weak-sensitive-data-hashing (HIGH). Its paths start
// at ListPreAuthKeys (the HSPreAuthKey slice, a type whose NAME reads as a
// secret) and run through the observed-state map reconcileFetch builds in
// jobs.go into this function, its only caller. False positive on two
...
//
// TRIP-WIRE: ground 1 holds only while the observed state stays metadata-only.
// If a secret is ever copied into the map reconcileFetch builds for this hash
// (jobs.go) — an actual pre-auth key, a token, an API key — or HashObserved
// gains a caller that passes one, re-open .github/codeql-register.tsv, because
// the hash would then cover material worth guessing at.
