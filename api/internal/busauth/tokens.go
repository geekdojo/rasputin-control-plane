...
// False positive: this input is not a password. GenerateToken below is the only
// source of what gets hashed, and it produces 32 bytes from crypto/rand — 256
...
//
// TRIP-WIRE: this rests entirely on the input being generated, never chosen. If
// HashToken is ever called on an operator-typed secret — a password, a
// passphrase, a PSK someone picks — this verdict is void and the finding
// becomes real. Today its only callers are GenerateToken and mint, both of
// which hash freshly generated bytes.
