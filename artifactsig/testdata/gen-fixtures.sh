...
# expired-signer.sig — a detached signature by a leaf whose validity window has
# already closed, for TestSignerExpiry. NOT regenerated by the block above: it
# needs a back-dated certificate, and it is byte-identical to the one in
# api/internal/releases/testdata/, whose gen-expired-fixture.sh produces it.
# Copy it across rather than minting a second one:
#
#   cp ../../api/internal/releases/testdata/expired-manifest.json.sig \
#      expired-signer.sig
