...
var metadataMinAgentVersion = map[string]string{
	...
	// nodeKeys (geekdojo/geekdojo-brain#514): entered when the field was
	// authored, before its release existed, as the NEXT release run after
	// v2026.09.4-dev.171 (the newest published control-plane release at the
	// time). If a release was cut between this entry and the one that ships
	// the field, an agent at that in-between version reads as "should report
	// and did not" rather than "predates the field"; confirm with
	// `git tag --contains` once the release exists and correct the floor.
	//
	// ⚠️ The floor says only that the agent CAN report. A node at or above it
	// that reports nothing has not necessarily failed: keys are reported only
	// over a pinned TLS bus connection (NodeKeysAcceptable), so a node still
	// on plaintext is silent by design.
